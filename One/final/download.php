...
//echo $imagetype;
  ...
  if($upload==1)//if the image format is correct
    { 
      if(move_uploaded_file($_FILES["filetoupload"]["tmp_name"], $target_name))//file uploaded
  	   {
        $filename=$email.$_FILES["filetoupload"]["name"];//concatenating file name with email-id
 ...
 // if($flag==1)
    ...
    function checksub($subjects)//function for checking subject-name 
    {
     $subs=trim($subjects);
     $flag=0;
     $arr=str_split($subs);
  //if((words.charCodeAt(j)>=65 && words.charCodeAt(j)<=90) || (words.charCodeAt(j)>=97 && words.charCodeAt(j)<=122))
    ...
    function checknum($number)//function for checking number-name
    {
      $num=trim($number);
      if(is_numeric($num))
        return 1;
